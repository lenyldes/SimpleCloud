...
- [x] 1.4 Add static regression test in `services/storage-service/cmd/main_test.go` verifying that `.github/workflows/ci.yml` deploy script starts with `set -e` (RED state).
...
- [x] 2.5 Add `set -e` directive to `.github/workflows/ci.yml` SSH deploy script to fail fast on errors (GREEN state).
