...
- [x] 2.1 Create `services/web-frontend/src/css/` and extract modular stylesheets:
...
- [x] 2.2 Update `services/web-frontend/src/index.html` to reference all 4 modular stylesheets in strict cascade order with cache-busting query parameter `?v=1.2.0`, and remove `services/web-frontend/src/styles.css`.
- [x] 2.3 Create `services/web-frontend/src/css/README.md` containing the in-place navigation guide, file responsibilities, and the Component-to-File lookup table.
- [x] 2.4 Update `docs/DESIGN_SPEC.md` to document the modular CSS architecture and file breakdown.
- [x] 2.5 Run test suite (`go test -v ./...` in `services/storage-service`) to verify GREEN state with 100% selector parity.
