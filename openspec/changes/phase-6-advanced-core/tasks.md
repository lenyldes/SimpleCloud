...
- [x] 1.1 Create migration `000003_folder_schema.sql` adding `folders` table and `files.folder_id` column with foreign keys and CASCADE rules
...
- [x] 2.2 Implement session cleanup goroutine worker in `internal/auth/service.go` to make tests GREEN
...
- [x] 3.2 Implement folder domain models, database queries, and HTTP handlers in `internal/handler/folder.go` to make tests GREEN
- [x] 3.3 Update file upload and deletion handlers in `internal/handler/file.go` to support `folder_id` and recursive folder deletion
...
- [x] 4.2 Implement pre-stream and stream quota check in `internal/handler/file.go` to make tests GREEN
