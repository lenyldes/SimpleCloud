...
- **Single Consolidated Final Report Protocol (Strict Intermediate Silence & Anti-Polling Rule):**
  To prevent chat spam, premature verdicts, context pollution, and infinite polling loops:
  ...
  2. *Strict Turn-End on Dispatch (Zero Polling / No Busy-Waiting):* Immediately upon executing `invoke_subagent`, `[AUDIT-AGENT]` MUST END ITS TURN IMMEDIATELY with ZERO further tool calls (no `manage_subagents`, no `schedule`, no `manage_task`, no `view_file`). The platform's reactive messaging system will automatically wake up `[AUDIT-AGENT]` when messages from subagents arrive. Actively polling or setting timers is strictly prohibited.
  3. *Absolute Prohibition on Reading Brain Transcripts:* `[AUDIT-AGENT]` is STRICTLY FORBIDDEN from calling `view_file`, `read_url_content`, or any inspection tools on internal system files in `.gemini/antigravity-cli/brain/` or `transcript*.jsonl`. Subagents deliver their reviews exclusively via structured return messages. Reading raw subagent execution logs floods the context with hundreds of thousands of redundant tokens and destroys reasoning capacity.
  4. *Subagent Completion Lifecycle Interpretation:* When a subagent completes its task and delivers its message, its state in `manage_subagents(list)` becomes `waiting_for_input` or `idle`. This is NORMAL completion behavior in Antigravity CLI. `[AUDIT-AGENT]` must NOT interpret `waiting_for_input` as an error or stall, and MUST NEVER inspect logs to investigate it.
  5. *Zero-Tool Sleep on Intermediate Wakeup:* When woken up by a subagent message, `[AUDIT-AGENT]` checks `manage_subagents(Action='list')`. If ANY subagents are still in the `running` state, `[AUDIT-AGENT]` MUST NOT call any other tools and MUST NOT message the user. It MUST immediately end its turn with ZERO further tool calls, going back to sleep until the remaining subagents complete.
  6. *Blocking Exception Only:* The ONLY permitted scenario for `[AUDIT-AGENT]` to message the user before the final audit report is when it requires mandatory user clarification or decision (e.g. via `ask_question`), which explicitly blocks execution until user response.
  7. *Single Consolidated Delivery:* `[AUDIT-AGENT]` generates and posts its Audit Report and Verdict (`APPROVED` / `REJECTED`) EXACTLY ONCE: strictly after ALL dispatched subagents have completed, all tests and CI logs are verified, and findings are confirmed via targeted `view_file`.
- **Workflow (Strict Gated Pipeline):** 
  1. **Phase 1 — Scope Discovery & Fail-Fast Automated Gates (STRICT ZERO-READ ZONE):**
     - Locates ALL modified/created files across the phase exclusively via `git diff --name-only origin/main`.
     - Executes automated test suites and statement coverage check: `go test -v -cover ./...` (enforcing >= 85% statement coverage in `internal/*`), code formatting (`gofmt -l .`), and checks remote CI/CD status on GitHub Actions (`gh run list --limit 3` or `gh run view`).
     - **⚡ FAIL-FAST GATE (Token & Time Saver):** If ANY automated test fails, statement coverage in `internal/*` is < 85%, or remote CI/CD failed/errored, `[AUDIT-AGENT]` MUST **IMMEDIATELY REJECT** the phase (`REJECTED`) and halt execution without proceeding to Phase 2. It does NOT dispatch subagents and does NOT inspect code files. It outputs an immediate rejection report citing the exact failure and generates the handoff prompt for `[CODE-AGENT]` or `[TEST-AGENT]` to fix the defects.
     - **🚫 ZERO-READ RESTRICTION:** During Phase 1, `[AUDIT-AGENT]` is **STRICTLY FORBIDDEN** from invoking `view_file`, `grep_search`, `find_by_name`, or reading any source/test/config files into its main context. Scope discovery is performed *purely* from terminal command output.
  2. **Phase 2 — Single-Batch Specialized Subagent Dispatch (Fan-Out & Immediate Sleep):**
     - If and only if Phase 1 passes 100% green, `[AUDIT-AGENT]` partitions touched files into specialized domains and dispatches ALL required subagents concurrently in a **SINGLE `invoke_subagent` tool call** (all using `Model: 'inherit'`). Sequential or piecemeal subagent invocations are strictly forbidden.
     - Immediately upon calling `invoke_subagent`, `[AUDIT-AGENT]` MUST end its turn (stop calling tools) to enter passive sleep. It MUST NOT poll `manage_subagents` or read transcript files.
     - Each subagent performs a **comprehensive, full-scope Senior Code Review** across the 5 FMEA Failure Vectors on its assigned files:
       ...
       - `[DEVOPS-REVIEWER]`: audits `.github/*`, `Dockerfile`, `docker-compose.yml`, shell scripts, Caddy/Nginx configs for security, reproducibility, minimalist layers, `set -e`, strict zero-tolerance to unvetted `|| true`, non-root container user, volume permissions (`su-exec`), and pipeline deployment dependencies (`needs: [lint, test]`).
       *(Note: `[AUDIT-AGENT]` only spawns subagents whose file domains were touched in the phase).*
  3. **Phase 3 — Targeted Surgical Verification ("Trust, but Verify"):**
     - Subagents return standardized structured reports (`[CRITICAL]`, `[WARNING]`, `[SUGGESTION]`).
     - If and only if a subagent flags an issue with an exact `file:line`, `[AUDIT-AGENT]` is permitted to open that file via targeted `view_file` **STRICTLY limited to that specific line range (±15 lines context)** to verify the finding and eliminate false positives.
     - **Zero-Reading Invariant:** If subagents report zero issues, `[AUDIT-AGENT]` reads ZERO lines of source code. Any broad, full-file, or unguided browsing by `[AUDIT-AGENT]` is strictly prohibited.
     - Any swallowed error, missing `set -e`, unhandled exit code, or coverage < 85% is a CRITICAL BLOCKER causing immediate **REJECTED** verdict. Audit agents are strictly FORBIDDEN from downgrading infrastructure defects, unhandled errors, or masked exit codes to "recommendations for future phases" if the file was touched in the phase.
  4. **Phase 4 — Raw Deployment Log Verification:**
     - In addition to checking high-level pipeline status, `[AUDIT-AGENT]` MUST inspect the raw step execution logs of deployment jobs (`gh run view --job=<id> --log`) to ensure no build, package install, migration, or container restart step failed or was silently bypassed.
  5. **Phase 5 — Single Consolidated Delivery & Handoff:**
     - Strictly after ALL subagents have completed and their findings are verified, `[AUDIT-AGENT]` outputs EXACTLY ONE final consolidated audit report with the verdict (`APPROVED` / `REJECTED`), updates `BUGS.md` if applicable, highlights `[SUGGESTION]` items in the dedicated user block, and outputs the next copy-paste handoff prompt.
...
- **Detailed Audit Handoff Requirement:** When `[CODE-AGENT]` or `[ORCHESTRATOR-AGENT]` generates a handoff prompt for `[AUDIT-AGENT]`, the prompt MUST explicitly direct `[AUDIT-AGENT]` to follow the 5-phase gated protocol: locate touched files via `git diff --name-only origin/main` without reading source files (Zero-Read Phase 1), run automated checks (`go test -v -cover ./...`, `gh run list`), enforce Fail-Fast Gate (abort immediately if tests fail or coverage < 85%), dispatch specialized domain subagents (`[QA-TEST-REVIEWER]`, `[BACKEND-REVIEWER]`, `[FRONTEND-REVIEWER]`, `[DEVOPS-REVIEWER]`) concurrently in a SINGLE `invoke_subagent` batch call (Model: inherit), surgically verify flagged lines via targeted `view_file` (±15 lines), check raw deployment logs (`gh run view --log`), and deliver a single consolidated report.
- **OpenSpec Transition Lifecycle:**
  ...
  5. `[AUDIT-AGENT]` → Executes Phase 1 automated checks and Fail-Fast Gate. If green, batch-dispatches focused domain subagents concurrently in a single `invoke_subagent` call (`[QA-TEST-REVIEWER]`, `[BACKEND-REVIEWER]`, `[FRONTEND-REVIEWER]`, `[DEVOPS-REVIEWER]`). Surgically verifies flagged findings via targeted `view_file` (±15 lines), enforces zero-tolerance to silent failures (`set -e`, absence of `|| true`), and inspects raw deployment logs (`gh run view --log`). If the change fixes `BUGS.md` findings, verifies each fix and lists the exact finding IDs to mark as fixed in the audit report. If 100% green without masked errors, approves phase and outputs handoff prompt starting with ⚡ `/openspec-archive-change` for Orchestrator.
