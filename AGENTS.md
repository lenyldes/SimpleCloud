...
- **Detailed Audit Handoff Requirement:** When `[CODE-AGENT]` or `[ORCHESTRATOR-AGENT]` generates a handoff prompt for `[AUDIT-AGENT]`, the prompt MUST explicitly direct `[AUDIT-AGENT]` to locate ALL changed `*.go` and `*_test.go` files via `git diff --name-only origin/main`, perform deep code review via `view_file`, and run `go test -v -cover ./...` to execute all tests and enforce 85%+ coverage.
