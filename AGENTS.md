...
- **Workflow:** 
  ...
  2. **Multi-Agent Specialized Code Review (1 Role = 1 Subagent):** To prevent context fatigue and attention dilution, `[AUDIT-AGENT]` MUST NOT read dozens of files into its own main context. Instead, it partitions touched files into specialized domains and dispatches focused subagents concurrently via `invoke_subagent` (all using `Model: 'inherit'`):
     - `[QA-TEST-REVIEWER]`: audits `*_test.go` for assertion depth (no empty `NoError` checks without payload validation), boundary conditions (0 bytes, quota limits, 401/413), test isolation (no wiping real DB admin), and absence of flaky `time.Sleep`.
     - `[BACKEND-REVIEWER]`: audits Go production code (`internal/*`, `cmd/*`, `.sql` migrations) for goroutine/file descriptor leaks, context propagation, tenant isolation (`WHERE user_id = $1`), strict error handling (no swallowed errors), and <= 450 lines limit.
     - `[FRONTEND-REVIEWER]`: audits `services/web-frontend/src/*` (HTML, JS, CSS) for XSS (`textContent` vs `innerHTML`), event listener cleanup, error modal handling (401, 413), and CSS BEM/modularity.
     - `[DEVOPS-REVIEWER]`: audits `.github/*`, `Dockerfile`, `docker-compose.yml`, shell scripts, Caddy/Nginx configs for `set -e`, strict zero-tolerance to unvetted `|| true`, non-root container user, volume permissions (`su-exec`), and pipeline fail-fast dependencies (`needs: [lint]`).
  ...
  5. **Synthesis & Handoff:** Synthesizes verified findings into the final audit report, updates `BUGS.md` if applicable, and outputs the next copy-paste handoff prompt.
