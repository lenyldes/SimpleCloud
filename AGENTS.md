...
- **BUGS.md & Audit Lifecycle Management:** `BUGS.md` in the project root tracks **active/open** audit findings and defects for the current development cycle. During active fixes, `[AUDIT-AGENT]` verifies resolved issues against `BUGS.md`. At phase/audit completion (`/openspec-archive-change`), when all findings in `BUGS.md` are verified as fixed (`✅`), `[ORCHESTRATOR-AGENT]` archives the full audit report into `docs/audits/YYYY-MM-DD-audit.md` (preserving 100% of historical details and traceability) and resets root `BUGS.md` to a lightweight active template (0 active findings).
